...
namespace sofs21
{
    void grpComputeDiskStructure(uint32_t ntotal, uint16_t itotal, uint16_t & itsize, uint32_t & dbtotal)
    {
        ...
        //if, at entry, the proposed value for itotal is 0, value ntotal/20
        ...
        uint16_t remainder = ntotal - itsize - 1;
        dbtotal = remainder - (uint16_t) ceil((double)remainder/(BlockSize*8));
